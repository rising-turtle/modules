...
//      cerr << CL_GREEN(__PRETTY_FUNCTION__ << ": updating old edge with new measurement") << endl;
  ...
  void CholOptimizer<PG>::solveAndUpdate(double** block, int r1, int c1, int r2, int c2){
    ...
    if (! ok) {
	  ...
	  for (typename PG::VertexIDMap::iterator it_vertex=_vertices.begin(); it_vertex!=_vertices.end(); it_vertex++)
	  {
		  typename PG::Vertex* v=_MY_CAST_<typename PG::Vertex*>(it_vertex->second);
			  cerr <<" "<<v->id()<<" ";
	  }
	  cerr<<endl;

	  for ( std::set<AISNavigation::Graph::Edge*>::iterator it_edge =_edges.begin();
		  it_edge!=_edges.end();
		  it_edge++){
			   AISNavigation::Graph::Edge* e=*it_edge;
			  cerr << " ("<<e->from()->id()<<","<<e->to()->id()<<") ";
	  }
