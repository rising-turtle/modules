	...
	while (!pSLAMClient->m_bStopSLAM)
	{
		if (pSLAMClient->m_bNewSyncDataArrive&&nAcc==0)
		{
			...
			if (nRtn==0)
			{
				...
				memcpy(pSLAMClient->m_dKeepCurPos,pSLAMClient->m_dCurPos,48);
			...
			{
				memcpy(pSLAMClient->m_dCurPos,pSLAMClient->m_dKeepCurPos,48);
				memcpy(pSLAMClient->m_pucSLAMData,&nRtn,4);
			}
			else if(nRtn<-1)
			{
				pSLAMClient->m_dCurPos[0]=WRONG_VALUE;
				pSLAMClient->m_dCurPos[1]=WRONG_VALUE;
				pSLAMClient->m_dCurPos[2]=WRONG_VALUE;
				pSLAMClient->m_dCurPos[3]=WRONG_VALUE;
				pSLAMClient->m_dCurPos[4]=WRONG_VALUE;
				pSLAMClient->m_dCurPos[5]=WRONG_VALUE;
				memcpy(pSLAMClient->m_pucSLAMData,&nRtn,4);
			}
