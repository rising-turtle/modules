...
Node::Node(const cv::Mat& visual,
    ...
    const boost::shared_ptr<pointcloud_type> point_cloud,
...
MatchingResult Node::matchNodePair(const Node* older_node){
  ...
  else 
  {
	...
	{ // mr.all_matches.size()/3
      //ROS_INFO("Found no valid trafo, but had initially %d feature matches",(int) mr.all_matches.size());

		// for debug
		//cout<<"bad Alignment form last frame with matched-inliers " << mr.inlier_matches.size()<<endl;
...
MatchingResult Node::matchLastNode(const Node* older_node, CPose3D& last_pose, CPose3D& curr_pose)
{
	...
	else 
	{
		...
		else  
		{
			...
			cout <<"FINAL_POSE After leastSquareErrorRigidTransformation6D!"<<endl;
			finalPose.output(std::cout);
