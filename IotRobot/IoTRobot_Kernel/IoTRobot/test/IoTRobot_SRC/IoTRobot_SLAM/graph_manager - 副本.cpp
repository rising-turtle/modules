...
//                ROS_INFO("Added Edge between %i and %i. Inliers: %i",mr.edge.id1,mr.edge.id2,(int) mr.inlier_matches.size());
    ...
    if (optimizer_->edges().size() > num_edges_before) { //Success
		...
		// remove the nodes out of range!!
		// TODO: remove is called when graph size exceed the threshold!
		cout<<"IN OR"<<endl;
		deleteOutRangeFrames(latest_pose[0], latest_pose[1], latest_pose[2], global_graph_radius);
		cout<<"OUT OR"<<endl;
		// remove the oldest node when graph size exceed the threshold!
		while(graph_.size()>global_graph_size){
			cout<<"before oldest node!"<<endl;
			Node* oldest_node = graph_.begin()->second;
			cout<<"before eraseNode!"<<endl;
			eraseNode(oldest_node);
			delete oldest_node;
			cout<<"Delete out list node"<<endl;
		}
...
void GraphManager::optimizeGraph(){
	cout<<"INTO OG: "<<graph_.size()<<"=="<<optimizer_->vertices().size()<<endl;
    std::clock_t starttime=std::clock();
