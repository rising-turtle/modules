...
IoTRobot_NetServer COpenNI::m_cNetServer;
...
void COpenNI::OpenNIInit(IoT_CallBackFuncSet callBackSet)
{
	...
	m_cNetServer.NetServer_Init(stNetServerCallBack);
	//printf("hahahaha");
}
...
UINT ThreadSendConf(LPVOID lpParam)
{
	...
	stMetConf.stVisulizationConf.cType=1;

	while (pOpenNi->m_cNetServer.NetServer_Conf(0,stMetConf)!=0)
	{
		Sleep(30);
	}
	return 0;
