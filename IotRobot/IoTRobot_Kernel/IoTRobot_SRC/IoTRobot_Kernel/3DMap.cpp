...
int C3DMap::C3DMapInit()
{
	...
	// for 2DMAP
	// 500*500/1024=157KB + 8b=165KB
	m_p2DMapBuf=new unsigned char[170*1024]; // 170KB
	return 0;
}
void C3DMap::Reset()
{
	// Reset PC and Content to render
	ResetBound();
	// if already swapped with Dynamic Cell Map, then, swap back
	if(m_bisswapped)
	{
		DynamicUnswap();
	}
	m_global_cells.clear();
	{
		boost::shared_ptr<pcl::PointCloud<pcl::PointXYZRGB> > pTmp(new pcl::PointCloud<pcl::PointXYZRGB> );
		C3DMap::m_global_cell_map.swap(pTmp);
	}
	{
		boost::shared_ptr<pcl::PointCloud<pcl::PointXYZRGB> > pTmp(new pcl::PointCloud<pcl::PointXYZRGB> );
		pGlobalDisplay.swap(pTmp);
	}
	{
		boost::shared_ptr<CDynamicArea<pcl::PointXYZRGB> > pTmp(new CDynamicArea<pcl::PointXYZRGB> );
		m_pcDynamicArea.swap(pTmp);
	}
		...
		if (p3Dmap->m_bOneRenderFrameReady)
		{
			...
			if(p3Dmap->m_Sessions.size()!=0)
			{
				...
				if(it!=p3Dmap->m_Sessions.end())
				{
					...
					//p3Dmap->m_p2DMap->UpdateMap((void*)((*it).get()));
					p3Dmap->m_Sessions.erase(it);
				}
				
				//p3Dmap->m_p2DMap->outStream(200,300,200,300,p3Dmap->m_p2DMapBuf);

				//unsigned int nrow;
				//unsigned int ncol;
				//unsigned char* pTmpBuf=p3Dmap->m_p2DMapBuf;
				//memcpy(&nrow,pTmpBuf,sizeof(unsigned int));
				//memcpy(&ncol,pTmpBuf+4,sizeof(unsigned int));
				//pTmpBuf+=8;
				////cout<<"NRow= "<<nrow<<endl;
				////cout<<"NCol= "<<ncol<<endl;
...
void C3DMap::Generate2DMap()
{
	m_p2DMap->UpdateMap((void*)(this));
}