...
BYTE* CCameraDS::QueryFrame()
{
	...
	if (size != m_nBufferSize)
	{
		...
		m_nBufferSize = size;
		//m_pFrame = new Mat(m_nHeight, m_nWidth, CV_8UC3);
		//m_pFrameTmp = new Mat(m_nHeight, m_nWidth, CV_8UC3);
		m_pFrameArr = new unsigned char[m_nHeight * m_nWidth * 3];
		
	}

	m_pSampleGrabber->GetCurrentBuffer(&m_nBufferSize, (long*)m_pFrameArr);
	//flip(*m_pFrame, *m_pFrameTmp, 0);
	int wholePointCount = m_nHeight * m_nWidth;
	int loopCount = wholePointCount / 2;
	unsigned char tmpByte;
	for (int i = 0; i < loopCount; i++)
	{
		tmpByte = m_pFrameArr[(wholePointCount - 1 - i) * 3 + 0];
		m_pFrameArr[(wholePointCount - 1 - i) * 3 + 0] = m_pFrameArr[i * 3 + 0];
		m_pFrameArr[i * 3 + 0] = tmpByte;

		tmpByte = m_pFrameArr[(wholePointCount - 1 - i) * 3 + 1];
		m_pFrameArr[(wholePointCount - 1 - i) * 3 + 1] = m_pFrameArr[i * 3 + 1];
		m_pFrameArr[i * 3 + 1] = tmpByte;


		tmpByte = m_pFrameArr[(wholePointCount - 1 - i) * 3 + 2];
		m_pFrameArr[(wholePointCount - 1 - i) * 3 + 2] = m_pFrameArr[i * 3 + 2];
		m_pFrameArr[i * 3 + 2] = tmpByte;
