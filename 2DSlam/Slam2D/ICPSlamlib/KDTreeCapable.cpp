	...
	std::vector<float>  &out_dist_sqr ) const
{
	...
	if ( KDTreeData.m_nTreeSize<N )
		THROW_EXCEPTION( "KD-tree must have at least %u points." )
	...
	std::vector<float>  &out_dist_sqr ) const
{
	...
	if ( !KDTreeData.m_nTreeSize) THROW_EXCEPTION("KD-tree must have at least %u point")
...
/*---------------------------------------------------------------
				kdTreeNClosestPointsWith3DIdx
---------------------------------------------------------------*/
void KDTreeCapable::kdTreeNClosestPointsWith3DIdx(
	float			x0,
	float			y0,
	float			z0,
	std::vector<float>  &out_x,
