				       ...
				       std::vector< std::vector<unsigned int> >& indexes) const
{
    ...
    for(unsigned int scale = 0; scale < m_scales.size(); scale++){
		...
		std::vector< std::vector<double> > weights(vertexNumber);
		for(int i=0;i<vertexNumber;i++)
			weights[i].resize(vertexNumber);
		std::vector<double> weightNormalizer(vertexNumber);
...
void CurvatureDetector::computeScaleBank()
{
    ...
    for(unsigned int i = 0; i < m_scales.size(); i++){
		m_scales[i] = m_baseSigma * pow(m_sigmaStep, (int)i);
    }
}
