...
const float   PM_D2R = M_PI/180.0; // degrees to rad
const float   PM_R2D = 180.0/M_PI; // rad to degrees
...
public:
	...
	_PARAM(std::string laser_name, int points,int fov, int max_range,int valid_points,\
		int scan_window,int corridor_thre):
...
public:
	_PM_SICK_LMS511():Base_PARAM("Sick_LMS511",361,180,5000,80,40,25.0){}
}PM_SICK_LMS511;

typedef struct _PM_SICK_LMS211 : public Base_PARAM{
public:
	_PM_SICK_LMS211():Base_PARAM("Sick_LMS211",181,180,1000,40,20,25.0){}
}PM_SICK_LMS211;
...
/*
hash["Sick_LMS511"] = ObtainParam<_PM_SICK_LMS511>();
Base_PARAM *get(string *name){
v = hash[name];
}
return (*v)()
*/
...
Base_PARAM* getParam(std::string laser_name){
	...
	if(_hash.size()==0){ // first time
		_hash["Sick_LMS511"] = ObtainParam<_PM_SICK_LMS511>;
		_hash["Sick_LMS211"] = ObtainParam<_PM_SICK_LMS211>;
	}
...
typedef struct _PMScan
{
	...
	_PMScan& operator=(_PMScan& ref){
		if(this!=&ref){
			t = ref.t; rx = ref.rx; ry = ref.ry; th = ref.th;
			if(np<ref.np){
				r = realloc(r,sizeof(float)*ref.np);
				x = realloc(x,sizeof(float)*ref.np);
				y = realloc(y,sizeof(float)*ref.np);
				bad = realloc(bad,sizeof(int)*ref.np);
				seg = realloc(seg,sizeof(int)*ref.np);
			}
			np = ref.np;
			memcpy(r,ref.r,sizeof(float)*np);
			memcpy(x,ref.x,sizeof(float)*np);
