...
float CPolarMatch::FMatch(PMScan* lsr, PMScan* lsa, bool bFirst)
{
	if(lsr == NULL || lsa == NULL){
		std::cout<<"Error in FMatch!"<<std::endl;
		return -1.0;
	}
	// first time lsr may not be processed
	// static bool bFirst = true;
	if(bFirst){
		pm_preprocessScan(lsr);
		bFirst = false;
	}
	// lsr has already been processed
	pm_preprocessScan(lsa);

	// match only considering the relative transform
	lsa->rx = lsr->rx = 0;
	lsa->ry = lsr->ry = 0;
	lsa->th = lsr->th = 0;

	bool psm_matched = true; 
	try{
		return pm_psm(lsr,lsa);
	}catch (int err){
		psm_matched = false;
	}
	if(!psm_matched){
		try{
			return pm_icp(lsr,lsa);
		}catch(int err){
			std::cout<<"frontend match failed!"<<std::endl;
		}
	}
	return -1.0;
}
...
void CPolarMatch::runlog(int run_num) // run slam using log data
{
	if(!m_bReady){
...
void CPolarMatch::runlogImproved(int run_num)	// rum slam using log data
{
	if(!m_bReady){
		cout<<"Laser is not initialized!"<<endl;
		return ;
