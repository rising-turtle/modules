...
{
	CSocketClient *pClient = static_cast<CSocketClient*>(param_);
	pClient->setThreadSendOn();
	pClient->startSendDataOffline();
	return 1;
}

bool CSocketClient::sendThreadStatus()
{
	return m_bSendOn;
}

void CSocketClient::setThreadSendOn()
{
	m_bSendOn = true;
}

void CSocketClient::setThreadSendOff()
{
	m_bSendOn = false;
}
...
bool CSocketClient::connect(const char* hip, int hport)
{
#ifdef WIN32
	if(m_bConneted) return true;
	// Start up Winsock ...
	WSADATA wsadata;
