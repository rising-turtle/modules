...
MOpenni2::MOpenni2() :
fps_(30), height_(480), width_(640),
device(new openni::Device),
m_depthStream(new openni::VideoStream),
m_colorStream(new openni::VideoStream),
depthVideoMode(new openni::VideoMode),
colorVideoMode(new openni::VideoMode),
...
MOpenni2::~MOpenni2()
{
	...
	if(m_depthStream) delete m_depthStream;
	if(m_colorStream) delete m_colorStream;
	if(depthVideoMode) delete depthVideoMode;
	if(colorVideoMode) delete colorVideoMode;
	if(m_depthFrame) delete m_depthFrame;
	if(m_colorFrame) delete m_colorFrame;
}
...
bool MOpenni2::init()
{
	...
	m_streams[0] = m_depthStream; // &m_depthStream;
	m_streams[1] = m_colorStream;// &m_colorStream;
