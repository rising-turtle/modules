	      ...
	      KDTreeData.m_nDim != nDims )
	{
		...
		if (N)
		{
			...
			KDTreeData.m_pDataTree=new ANNkd_tree(
				...
				KDTreeData.m_nDim ,
				1);//this number is the size of each bucket in Kd-tree
		}
	...
	float		  &out_dist_sqr ) const
{
	...
	if ( !KDTreeData.m_nTreeSize ) cout<<"There are no points in the KD-tree."<<endl;
	...
	float   y0 ) const
{
	...
	if ( !KDTreeData.m_nTreeSize ) cerr<<"There are no points in the KD-tree."<<endl;
	...
	float	&out_dist_sqr2 ) const
{
	...
	if ( KDTreeData.m_nTreeSize<2 ) cerr<<"Map must have at least 2 points."<<endl;
	...
	std::vector<float>  &out_dist_sqr ) const
{
	...
	if ( KDTreeData.m_nTreeSize<N )
		cerr<<"KD-tree must have at least "<<N<<" points."<<endl;
...
---------------------------------------------------------------*/
size_t KDTreeCapable::kdTreeClosestPoint3D1(
	float   x0,
	float   y0,
	float   z0,
	float   &out_x,
	...
	float	&out_dist_sqr ) const
{
	...
	//if ( !KDTreeData.m_nTreeSize) cerr<<"KD-tree must have at least "<<N<< " points."<<endl;
  ...
  //  KDTreeData.m_pDataTree->annkSearch(
	...
	KDTreeData.m_pDataTree->annkSearch1(
		KDTreeData.m_QueryPoint,
		out_x,out_y,out_z,
		out_dist_sqr
		);
