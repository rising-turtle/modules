...
class ANNmin_k {
	...
	struct gravity_node{// node as return value when using gravity_point as approximately evaluation
	public:	
		gravity_node(){valid = false;}// Whether this node contain valid gravity_node
		PQKkey key;
		bool valid;
		float gravity_p[3];
	};
...
public:
	gravity_node *g_node;          //  record three coordinates' value
...
public:
	ANNmin_k(int max)					// constructor (given max size)
		{
			...
			g_node = new gravity_node; // store gravity node
		}
	...
	inline void insert(
		...
		int Dim =3)
	{
		//g_node->gravity_p = new ANNcoord[Dim];
		for(int d=0;d<Dim;d++)
			g_node->gravity_p[d] = p[d];
		g_node->key = kv;
		g_node->valid = true;
	}
};
