	...
	double				eps)			// error bound (ignored)
{
	...
	while (ANNprBoxPQ->non_empty() &&
		...
		// JLBC: Modified to avoid dereferencing type-punned pointer warning
		void *np_ptr;
										// extract closest box from queue
		ANNprBoxPQ->extr_min(box_dist, np_ptr );//  (void *&) np);

		ANNkd_ptr np= (ANNkd_ptr)np_ptr;					// next box from prior queue
