  ...
  laser_count_(0), transform_thread_(NULL)
{
  ...
  path_pub_ = node_.advertise<nav_msgs::Path>("path", 1, true); //liu
  ss_ = node_.advertiseService("dynamic_map", &SlamGMapping::mapCallback, this);
  scan_filter_sub_ = new message_filters::Subscriber<sensor_msgs::LaserScan>(node_, "scan", 5);
  scan_filter_ = new tf::MessageFilter<sensor_msgs::LaserScan>(*scan_filter_sub_, tf_, odom_frame_, 5);
  scan_filter_->registerCallback(boost::bind(&SlamGMapping::laserCallback, this, _1));

  // added by David Z, for syn query   
  ack_pub_ = node_.advertise<std_msgs::Bool>("ack", 1);
  syn_sub_ = node_.subscribe("/syn", 1, &SlamGMapping::subscribeSYN, this);

  // added by David Z, for save path 
  path_save_pub_ = node_.advertise<std_msgs::Bool>("save_path_ack", 1); 
  path_save_sub_ = node_.subscribe("/save_path_syn", 1, &SlamGMapping::pathSaveSub, this);

  transform_thread_ = new boost::thread(boost::bind(&SlamGMapping::publishLoop, this, transform_publish_period));
}

void SlamGMapping::publishACK()
{
...
void SlamGMapping::subscribeSYN(const std_msgs::BoolPtr& syn)
{
    publishACK();
}

void SlamGMapping::pathSaveSub(const std_msgs::BoolPtr& syn)
{
    savePath();
    std_msgs::BoolPtr ok(new std_msgs::Bool); 
    ok->data = true;
    path_save_pub_.publish(ok); 
}

void SlamGMapping::savePath()
{
    ofstream ouf("gmap_path.txt");
    GMapping::GridSlamProcessor::Particle best = 
        gsp_->getParticles()[gsp_->getBestParticleIndex()];
    
    vector<double> px, py, ptheta;
            ...
            n = n->parent)
    {
        ...
        px.push_back(n->pose.x);
        py.push_back(n->pose.y);
        ptheta.push_back(n->pose.theta);
        // ouf<<std::fixed<<n->pose.x<<" "<<n->pose.y<<" "<<n->pose.theta<<endl;
    }
    assert(px.size() == pose_time_.size());   
    int N = px.size();
    std::reverse(px.begin(), px.end());
    std::reverse(py.begin(), py.end());
    std::reverse(ptheta.begin(), ptheta.end());
    for(int i=0; i<N; i++)
    {
        ouf<<std::fixed<<pose_time_[i]<<" "<<px[i]<<" "<<py[i]<<" "<<ptheta[i]<<" "<<endl;
    }
}
...
{
  // static ofstream outf("gmapping_slam.txt");
  // static int num = 0;
    
  // static ofstream outf2("odo_gmapping.txt");  

  if(!getOdomPose(gmap_pose, scan.header.stamp))
     return false;
  // outf2<<scan.header.stamp<<" : "<<gmap_pose.x<<"\t"<<gmap_pose.y<<"\t"<<gmap_pose.theta<<endl;
  ...
  ROS_DEBUG("scanpose (%.3f): %.3f %.3f %.3f\n",
            ...
            gmap_pose.theta);
            */
  /*vector<double> tmp_reading; 
  tmp_reading.insert(tmp_reading.begin(), reading.begin(), reading.end());
  GMapping::OrientedPoint tmp_pose = gmap_pose;*/

  bool ret =  gsp_->processScan(reading);
  if(ret)
  {
  /*
      outf<<++num<<" input : "<<gmap_pose.x<<"\t"<<gmap_pose.y<<"\t"<<gmap_pose.theta<<endl;
      for(int i=0; i< reading.size(); i++)
      {
          outf<<reading[i]<<" ";
          if(i!=0 && i% 15 == 0) outf<<endl;
      }
      outf<<endl;  
      GMapping::OrientedPoint mpose = gsp_->getParticles()[gsp_->getBestParticleIndex()].pose;
      outf<<"output: "<<mpose.x<<"\t"<<mpose.y<<"\t"<<mpose.theta<<endl;  
  */
  }
  return ret;
...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  {
    pose_time_.push_back(scan->header.stamp.toSec());
...
//TODO:liu: publish path
		  ...
		  n = n->parent)
  {
	  ...
	  pose.header.stamp.fromSec(n->reading->getTime());
	  pose.pose.position.x = n->pose.x;
	  pose.pose.position.y = n->pose.y;
	  pose.pose.position.z = 0;
	  tf::Quaternion q;
	  q.setRPY(0,0,n->pose.theta);
